...
public:
    inline TArray() : Data(nullptr), Count(0), Max(0) {};

    inline TArray(const T& item) : Data(new T[1]), Count(1), Max(1) {
        Data[0] = item;
    };
    ...
    inline TArray(const std::vector<T>& vec) {
        if (vec.size() > static_cast<size_t>(INT32_MAX)) {
            throw std::runtime_error("Vector size exceeds maximum size for TArray");
        }
        Data = new T[vec.size()];
        Count = static_cast<int32_t>(vec.size());
        Max = static_cast<int32_t>(vec.size());
